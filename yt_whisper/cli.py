...
def main(argv=None):
    ...
    parser.add_argument("--temperature", default=None, help="0..1 or comma-separated fallback sequence")
    ...
    try:
        options = decoding_options(
            ...
            clip_timestamps=args.clip_timestamps, beam_size=args.beam_size,
        ...
        for source in args.video:
            with prepare_audio(source, storage) as audio:
                result = engine.transcribe(audio.path, args.model, args.language, args.task, args.device,
                                           **options)
                for path in save_result(result, audio, args.output_dir, formats, args.break_lines):
                    print(f"Saved: {path}")
